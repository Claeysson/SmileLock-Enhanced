...
public protocol PasswordInputCompleteProtocol: AnyObject {
    ...
    func cancelAuthentication(_ passwordContainerView: PasswordContainerView)
}
...
open class PasswordContainerView: UIView {
    ...
    //MARK: Property
    open var deleteButtonLocalizedTitle: String = "Delete" {
        didSet {
            setDeleteButtonTitle()
        }
    }
    
    open var cancelButtonLocalizedTitle: String = "Cancel" {
        didSet {
            setDeleteButtonTitle()
        }
    }
    ...
    open class func create(withDigit digit: Int) -> PasswordContainerView {
        ...
        //let nib = UINib(nibName: "PasswordContainerView", bundle: Bundle.smilelock_enhanced)
        let view = Bundle.module.loadNibNamed("PasswordContainerView", owner: self)?.first as! PasswordContainerView
    ...
    @IBAction func deleteInputString(_ sender: AnyObject) {
        ...
        guard inputString.count > 0 && !passwordDotView.isFull else {
            self.delegate?.cancelAuthentication(self)
            return
        }
...
private extension PasswordContainerView {
    ...
    func setDeleteButtonTitle() {
#if swift(>=3.2)
        if inputString.count == 0 {
            setDeleteButtonTitleText(string: cancelButtonLocalizedTitle)
        } else {
            setDeleteButtonTitleText(string: deleteButtonLocalizedTitle)
        }
#else
    ...
    private func setDeleteButtonTitleText(string: String) {
